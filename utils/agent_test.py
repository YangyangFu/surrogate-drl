...
class Q_GP():
    ...
    def learn(self, X, y, num_steps = 100):
        self.model = GPRegressionModel(train_x = X, train_y = y, likelihood = self.likelihood, mean_module = self.mean_module, base_covar_module = self.base_covar_module, covar_module = self.covar_module).to(device)
...
class Q_GP_multi():
    def __init__(self, input_dim, output_dim, lr, device):
        ...
        self.lr = lr
        self.device = device
        self.model = [Q_GP(input_dim, lr, device) for i in range(output_dim)]
    
    def learn(self, X, Y, num_steps):
        for i in range(self.n):
            self.model[i].learn(X, Y[:, i], num_steps=num_steps)
        return None
...
class Agent:
    def __init__(self, env, gamma, batch_size, ReplayBuffer_size, device) -> None:
        self.env = env
        self.device = device
        self.current_model = Q_GP_multi(env.observation_space.shape[0], env.action_space.n)
        self.target_model  = Q_GP_multi(env.observation_space.shape[0], env.action_space.n)
        self.update_target()
    ...
    def compute_td_loss(self, batch_size):
        ...
        self.current_model.learn(state, expected_q_value)
...
if __name__ == "__main__":
    ...
    y = 1000 * torch.sin(torch.cat([torch.unsqueeze(torch.mean(X, dim = 1),dim=1), torch.unsqueeze(torch.mean(X, dim = 1)*2, dim=1)], dim=1)) 
    + torch.distributions.normal.Normal(0.0, 0.2).sample(sample_shape=(N,2))

    X1 = torch.distributions.uniform.Uniform(0.0, 1.0).sample(sample_shape=(N,10))
    y1 = 1000 * torch.sin(torch.cat([torch.unsqueeze(torch.mean(X, dim = 1),dim=1), torch.unsqueeze(torch.mean(X, dim = 1)*2, dim=1)], dim=1)) 
    + torch.distributions.normal.Normal(0.0, 0.2).sample(sample_shape=(N,2))
