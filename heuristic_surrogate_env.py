...
from surrogate_model.HVAC_env_linear import Gym_Wrapper_HVAC
...
class Agent_h:
    def __init__(self, gamma, batch_size, device) -> None:
        self.env = Gym_Wrapper_HVAC(device = device, trainable = False)
        self.device = device
    ...
    def save(self, model_path):
        torch.save(self.current_model.state_dict(), model_path + "surrogate_current_model.pt")
        torch.save(self.target_model.state_dict(), model_path + "surrogate_target_model.pt")

    def load(self, model_path):
        self.current_model.load_state_dict(torch.load(model_path + "surrogate_current_model.pt"))
        self.target_model.load_state_dict(torch.load(model_path + "surrogate_target_model.pt"))


class heuristic:
    def __init__(self, train_env, device, printable = False, load_agent = False) -> None:
        
        #device = torch.device('cuda')
        batch_size = args.batch_size
        gamma      = args.gamma
        
        self.env = Gym_Wrapper_HVAC(device, trainable=train_env)
...
def test_agent(file_path = "./history_data/", agent = None):
    ...
    return plot_one_ep_normalized(num_zone = 1, history_Z_T = l_indoor, history_Env_T = l_outdoor, his_cost = c, num_days = args.num_of_days, fig_path_name = './heu_surrogate_test_data_simulation.png')
